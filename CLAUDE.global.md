...
- 用户说"用 agent team / 团队 / 多代理 / 分头做"，或任务需要 ≥ 3 个独立模块并行时：**先调用 `agent-team` 技能**（`Skill agent-team`），按它的部门表、完整性门禁、模型分级执行。S 级小任务不要开 team，直接做并说明原因。
...
- 看板 http://127.0.0.1:7788/ 由 SessionStart 钩子常驻（代码 `~/.claude/team-board/`）。派 ≥ 2 个子代理前先运行 `python ~/.claude/team-board/ensure.py --open`（保证服务在跑并在用户浏览器打开看板；桌面版还可再用浏览器工具打开同一地址）。
- 成员回报后，在对话里用一两句话转述每个人的结论，不要只说"完成了"。完整性门禁不满足（缺质量部/文档部）不得宣布完成。
